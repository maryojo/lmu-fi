...
export async function middleware(request: NextRequest) {
  ...
  const {
   ...
   // Handle login success conditional routing
  if (request.nextUrl.pathname === '/login' && user) {
    // User is logged in but on login page - redirect based on your conditions
    const url = request.nextUrl.clone();
    
    // Add your conditional logic here based on user data
    let isCompletedLastQuiz = user?.user_metadata.lastQuizStarted === user?.user_metadata.lastQuizCompleted;
    if (user?.user_metadata.isOnboarded === false && user?.user_metadata.userRole === "student") {
      if (user?.user_metadata.attemptedQuiz === true && !isCompletedLastQuiz) {
        url.pathname = "/reset-onboarding"
      } else {
